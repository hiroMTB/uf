...
void cApp::update(){
    ...
    for(int i=0; i<mPoints.getNumVertices(); i++ ){
        ...
        Vec3f noise = mPln.dfBm(x, y, cs[i].a ) * 0.1;
        mVelocity[i] = mVelocity[i]*0.9 + (vel+noise)*0.1;
...
void cApp::draw(){
    ...
    gl::color( 255, 0, 0 );
    gl::drawStrokedCube( Vec3f(0,0,0), Vec3f(200,200,200) );

    glBegin( GL_LINES );
    glVertex2f( 1080*3*0.5/3, 0 );
    glVertex2f( 1080*3*0.5/3, 1920 );
    glVertex2f( 1080*3*0.5/3*2, 0 );
    glVertex2f( 1080*3*0.5/3*2, 1920 );
    glEnd();
